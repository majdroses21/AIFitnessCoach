<template>
  <NuxtLayout name="gym">
    <v-container class="pb-13">
      <v-row>
        <v-col lg="4" col="12">
          <p calss="text-primary">Hello</p>
          <h1>{{ store.user.personalInfo.name }} </h1>

        </v-col>
        <v-spacer></v-spacer>
        <v-col lg="6" cols="12">
          <v-skeleton-loader v-if="loader" type="card" class="mx-auto" max-width="500"></v-skeleton-loader>
          <v-card v-if="!loader" class="mx-auto" max-width="500">
            ...
            <v-card-text>
              <v-form ref="form" @submit.prevent="saveProfile">
                ...
                <v-text-field v-model="profile.height" label="Tall" type="number" outlined></v-text-field>
